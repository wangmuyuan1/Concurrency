...
public class Master extends UntypedPersistentActor {
  ...
  public void onReceiveCommand(Object cmd) throws Exception {
    if (cmd instanceof MasterWorkerProtocol.RegisterWorker) {
      ...
      if (workState.hasWork()) {
        final String workerId = ((MasterWorkerProtocol.WorkerRequestsWork) cmd).workerId;
        ...
        if (state != null && state.status.isIdle()) {
          ...
          persist(new WorkState.WorkStarted(work.workId), new Procedure<WorkState.WorkStarted>() {
            public void apply(WorkState.WorkStarted event) throws Exception {
      ...
      if (workState.isDone(workId)) {
        ...
        persist(new WorkState.WorkCompleted(workId,((MasterWorkerProtocol.WorkIsDone) cmd).result), new Procedure<WorkState.WorkCompleted>() {
          public void apply(WorkState.WorkCompleted event) throws Exception {
            workState = workState.updated(event);
      ...
      if (workState.isInProgress(workId)) {
        ...
        persist(new WorkState.WorkerFailed(workId), new Procedure<WorkState.WorkerFailed>() {
          public void apply(WorkState.WorkerFailed event) throws Exception {
            workState = workState.updated(event);
            notifyWorkers();
          }
        });
      ...
      if (workState.isAccepted(workId)) {
        ...
        persist(new WorkState.WorkAccepted((Work) cmd), new Procedure<WorkState.WorkAccepted>() {
          public void apply(WorkState.WorkAccepted event) throws Exception {
        ...
        if (state.status.isBusy()) {
          if (state.status.getDeadLine().isOverdue()) {
            ...
            persist(new WorkState.WorkerTimedOut(state.status.getWorkId()), new Procedure<WorkState.WorkerTimedOut>() {
              public void apply(WorkState.WorkerTimedOut event) throws Exception {
                workState = workState.updated(event);
                notifyWorkers();
              }
            });
